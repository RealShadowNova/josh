...
import type { BaseJoshProviderOptions } from '../types/BaseJoshProviderOptions';
import type { Josh } from './Josh';

export abstract class BaseJoshProvider<K = unknown> {
  public name: string;
  ...
  public constructor({ name, options, instance }: BaseJoshProviderOptions<K>) {
    this.name = name;
  ...
  public abstract set(key: string, path: string, value: K): Promise<BaseJoshProvider>;
  public abstract setMany(data: Record<string, Partial<K>>, overwrite: boolean): Promise<BaseJoshProvider>;

  public abstract delete(key: string, path: string): Promise<BaseJoshProvider>;
  public abstract clear(): Promise<BaseJoshProvider>;
  public abstract deleteMany(keysOrPaths: string[]): Promise<BaseJoshProvider>;

  public abstract push<V = K>(key: string, path: string, value: V, allowDupes: boolean): Promise<BaseJoshProvider>;
  public abstract remove<V = K>(key: string, path: string, valueOrFunction: ((value: V) => Awaited<boolean>) | V): Promise<BaseJoshProvider>;

  public abstract inc(key: string, path: string): Promise<BaseJoshProvider>;
  public abstract dec(key: string, path: string): Promise<BaseJoshProvider>;
  ...
  public abstract math(key: string, path: string, operation: string, operand: number): Promise<BaseJoshProvider>;
